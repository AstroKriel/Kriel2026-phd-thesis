...
While a galaxy's mass is largely concentrated in stars, planets, and black holes, most of its volume is filled by diffuse gas. Much of this gas is ionised and therefore in a plasma state, where a sea of electrons and ions can drift relative to one another, driving electric currents that in turn generate magnetic fields. Magnetic fields are therefore a natural consequence of plasma dynamics in galaxies, and unlike collisional gas interactions, allow plasmas to influence parts of flows far from where currents are produced. While these fields are not visible to our eyes, we have techniques that can reveal them (see \S\ref{sec:how-we-see}), and have shown that they not only thread an enormous range of scales in galaxies, from star-forming regions to large-scale outflows, but are already present in the first galaxies (see \S\ref{sec:what-we-see}).
...
In the remainder of this introduction we provide a broad overview of the observations and theory that are relevant to this thesis. We begin by describing how magnetic fields are inferred and what is observed in and around galaxies, and then turn to the dynamical processes that amplify and reorganise fields. We close this introduction, in \S\ref{sec:intro:roadmap}, by framing the questions tackled in this thesis against this background.

\input{chapters/thesis/introduction/seeing}

\input{chapters/thesis/introduction/modelling}

\section{Thesis Roadmap and Remaining Lacunae}
\label{sec:intro:roadmap}

The remainder of this thesis is organised around three paper-chapters. The first two chapters are published papers, and explore aspects of SSD amplification with particular emphasis on the role of compressibility. The final chapter is a code-development paper in the final stages of preparation, wherein we implement ideal MHD in a new astrophysical simulation code. Below we briefly motivate each chapter; each chapter also includes a dedicated introduction that provides further context and an overview of the relevant literature.
...
Finally, \uline{\S\ref{chapter:quokka} implements a new ideal-MHD constrained-transport (CT) scheme in the \textsc{Quokka} codebase}. Following the dynamical roads discussed in \S\ref{sec:evolve:paths}, it is clear that a single, self-consistent simulation of galaxy evolution requires resolving a wide dynamic range, from $\sim 10 \,\mathrm{kpc}$ down to $\sim 10 \,\mathrm{pc}$. This has been difficult to achieve with existing CPU-based codes, but is now becoming practical with next-generation, GPU-accelerated supercomputers, which \textsc{Quokka} was developed for from its outset. Until now, however, \textsc{Quokka} has lacked an MHD module. In \S\ref{chapter:quokka} we discuss our implementation of a new CT-based scheme, and validate that it  is both accurate enough for these applications, while remaining efficient enough to scale to the required resolutions.
